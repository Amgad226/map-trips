...
export default function EditParticipantView({ participant, handleUpdate }: Props) {
  const { t } = useTranslation();
  const router = useRouter();
  const [error, setError] = useState("");
  const [loading, setLoading] = useState(false);
  const [preview, setPreview] = useState<string | null>(
    participant.image ? getProxyUrl(participant.image) : null
  );
  ...
  return (
    <div className="max-w-xl">
      <h1 className="text-2xl font-bold text-foreground mb-6">{t("admin.editTrip")}</h1>
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex items-center gap-4 pt-4">
          ...
          >
            {loading ? t("tripForm.saving") : t("tripForm.updateTrip")}
          </button>
          <button
            type="button"
